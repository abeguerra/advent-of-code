public class Day2 {
  ...
  public static int calculateCheckSumMod(String[] input) {
    int checkSum = 0;
    ...
    for (String i : input) {
      ...
      for (int j = 0; j < nums.length; j++) {
        for (int k = j+1; k < nums.length; k++) {
          int numAtPos1 = Integer.parseInt(nums[j]);
          int numAtPos2 = Integer.parseInt(nums[k]);
          if (Math.max(numAtPos1, numAtPos2) % Math.min(numAtPos1, numAtPos2) == 0) {
            int temp = Math.max(numAtPos1, numAtPos2) / Math.min(numAtPos1, numAtPos2);
            checkSum += temp;
          }
        }
      }
  ...
  public static void main(String[] args) {
    ...
    System.out.println(calculateCheckSumMod(input));
  }
}