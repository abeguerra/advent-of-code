...
public class Day4 {
  ...
  public static boolean isValid2(String passphrase) {
    String[] tokens = passphrase.split(" ");
    HashSet<HashMap<Character, Integer>> tokenSet = new HashSet<>();
    for (String token : tokens) {
      ...
      Iterator<HashMap<Character, Integer>> iter = tokenSet.iterator();
      while (iter.hasNext()) {
        if (iter.next().equals(charMap)) {
          return false;
        }
      }
      tokenSet.add(charMap);
    }
    return true;
  }
...
"inc mpys mzqmcwx vryz ibqrzc pmsy fat rojpxwy rcbqzi gjef"
    ...
    // System.out.println("Enter a passphrase or 0 to exit");

    // String passphrase;
    // while(!(passphrase = scanner.nextLine()).equals("0")) {
    //   System.out.println(isValid(passphrase) ? "PASS" : "FAIL");
    // }
